import { ENGrid, UpsellPseudoSelectorOptions, UpsellPseudoSelectorOptionsDefaults, UpsellBase } from "./";
...
export class UpsellPseudoSelector extends UpsellBase {
  ...
  constructor() {
    ...
    this.options = { ...UpsellPseudoSelectorOptionsDefaults, ...options };

    if (!this.shouldRun()) {
      if (ENGrid.debug) console.log("UpsellPseudoSelector script should NOT run");
    ...
    if(siblingField) {
      const pseudoSelectorFieldWrapper = document.createElement("div") as HTMLDivElement;
    ...
    const pseudoSelectorField = document.querySelector(`input[name='${this.options.pseudoSelectorFieldName}']`) as HTMLFormElement;
  ...
  private hidePseudoselector() {
    const pseudoSelectorFieldWrapper = document.querySelector(`.en__field--monthly_upsell_checkbox`) as HTMLElement;
  ...
  private shouldRun() {
    return (
      'EngridPseudoSelectorUpsell' in window &&
