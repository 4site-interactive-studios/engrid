...
export class Modal {
    ...
    openUpsell() {
        ...
        // Only open Upsell Modal if Frequency == onetime & if the Modal is closed
        if (freq == "onetime" && this.overlay.classList.contains("is-hidden")) {
    ...
    open(modal) {
        ...
        if (hideModal && !this.debug)
            return;
        const overlayContent = this.overlay.querySelector("#enModalContent");
