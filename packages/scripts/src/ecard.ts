...
export class Ecard {
  ...
  private altsAndArias() {
    ...
    if (ecardRecipients) {
      const recipientName = ecardRecipients.querySelector(
        ".en__ecardrecipients__name"
      );
      if (recipientName) {
        this.coupleLabelAndInput(recipientName, "Recipient Name");
      }
      const recipientEmail = ecardRecipients.querySelector(
    ...
    const ecardFutureDelivery = document.querySelector(
      ".en__ecardrecipients__futureDelivery"
    );
    if (ecardFutureDelivery) {
      this.coupleH2AndInput(
        ecardFutureDelivery,
        "Schedule your eCard for future delivery"
      );
    }
    ...
    if (previewModal) {
      ...
      const closeButton = previewModal.querySelector(
        ...
        mutations.forEach((mutation) => {
          if (
            ...
            if (target.classList.contains("preview--show")) {
              ...
              const focusableElements = previewModal.querySelectorAll(
                'iframe, a[href], area[href], button:not([disabled]), object, embed, [tabindex="0"]'
              );
              ...
              if (previewButton) {
                (previewButton as HTMLElement).focus();
              }
            }
          }
        });
  ...
  private altsAndAriasEcardItemsList(list: Element) {
    // if there's a sibling h2, use its text as the aria-label for the list
    const h2 = list.previousElementSibling as HTMLHeadingElement;
    if (h2 && h2.tagName === "H2") {
    ...
    const thumbs = Array.from(
      list.querySelectorAll(".en__ecarditems__thumb")
    ) as HTMLElement[];
      ...
      mutations.forEach((mutation) => {
        if (
          ...
          if (target.classList.contains("thumb--active")) {
            ...
            thumbs.forEach((t) => {
              if (t !== target) t.setAttribute("tabindex", "-1");
            });
          } else {
  ...
  private coupleLabelAndInput(parent: Element, labelText: string) {
  ...
  private coupleH2AndInput(parent: Element, labelText: string) {
