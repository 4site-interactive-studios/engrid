...
**fieldOptInSelectorTargetLocation**: A string that is set to `'before'`, `'after'`, or `'rightSide'`. When set to `'rightSide'`, the target is wrapped in a flex container (with an 8px gap) and the opt-in element is appended into that same wrapper so it sits on the right side of the target element. The wrapper uses `flex-wrap`, so on narrow screens (e.g. mobile) where both elements don't fit on one line, the opt-in element automatically wraps below the target. The wrapper inherits the target's original `margin-top` and `margin-bottom` so the surrounding layout is preserved; the target's own vertical margins are zeroed out to avoid double-spacing inside the flex row. When the opt-in element wraps below the target, its `margin-left` is set to the target's `padding-left` so it stays left-aligned with the target's content. Alignment is recomputed whenever the wrapper resizes (via `ResizeObserver`, where supported). Defaults to 'after'.

> **Note on `'rightSide'`:** To achieve this layout, the target element is moved into a newly created flex `<div>` wrapper (`.rememberme-right-side-wrapper`). This changes the target's position in the DOM, which can affect theme CSS that relied on the target's original location — for example `>` child selectors from the old parent, or `+`/`~` sibling combinators. Full-width fields will also be compressed to share the row as flex items. If you use `'rightSide'`, verify the surrounding layout and adjust theme CSS as needed.
...
**fieldClearSelectorTargetLocation**: A string that is set to `'before'`, `'after'`, or `'rightSide'`. When set to `'rightSide'`, the target is wrapped in a flex container (with an 8px gap) and the "Clear Autofill" link is appended into that same wrapper so it sits on the right side of the target element. The wrapper uses `flex-wrap`, so on narrow screens (e.g. mobile) where both elements don't fit on one line, the link automatically wraps below the target. The wrapper inherits the target's original `margin-top` and `margin-bottom` so the surrounding layout is preserved; the target's own vertical margins are zeroed out to avoid double-spacing inside the flex row. When the link wraps below the target, its `margin-left` is set to the target's `padding-left` so it stays left-aligned with the target's content. Alignment is recomputed whenever the wrapper resizes (via `ResizeObserver`, where supported). Defaults to 'before'.

> **Note on `'rightSide'`:** As with the opt-in element, the target is moved into a new flex `<div>` wrapper (`.rememberme-right-side-wrapper`), which changes its position in the DOM. See the note under `fieldOptInSelectorTargetLocation` for the theme-CSS implications.
...
- **Clickable segment(s) via `{...}`**: Any text wrapped in curly braces `{...}` becomes a clickable "clear" link, while the text outside the braces is rendered as plain, non-clickable text. Only the braced text carries the clickable styling (the `label-tooltip` class), so surrounding text won't look clickable. You can include more than one `{...}` segment — each becomes its own clear link and clicking any of them clears the autofilled data. Empty braces (`{}`) are ignored (no empty link is rendered). When no braces with content are present, the entire element stays clickable (legacy behavior).
  ...
  Note: The opt-in tooltip shows a plain-text version of this label — the `$username` token is resolved and the `{...}` markers are stripped, so braces and the raw token never appear in the tooltip.
