...
- **Clickable segment via `{...}`**: The first occurrence of text wrapped in curly braces `{...}` becomes the clickable "clear" link, while the text outside the braces is rendered as plain, non-clickable text. Only that braced segment carries the clickable styling (the `label-tooltip` class), so surrounding text won't look clickable. Any additional `{...}` pairs in the label remain as literal text (braces included). When no braces are present, the entire element stays clickable (legacy behavior).
